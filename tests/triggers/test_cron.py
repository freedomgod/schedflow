...
def test_start_end_times_string(timezone, monkeypatch):
    monkeypatch.setattr(
        "schedflow.triggers.cron.get_localzone", Mock(return_value=timezone)
    )
    trigger = CronTrigger(
...
def test_cron_bad_kwarg(timezone):
    ...
    with pytest.raises(TypeError):
        CronTrigger(second=0, third=1, timezone=timezone)
    # Invalid values still surface as TriggerValidationError.
