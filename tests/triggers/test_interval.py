...
def test_invalid_interval(timezone):
    # Triggers use explicit keyword-only parameters: legacy positional
    # interval strings are rejected at the signature level.
    with pytest.raises(TypeError):
        IntervalTrigger("1-6", timezone=timezone)


def test_start_end_times_string(timezone, monkeypatch):
    monkeypatch.setattr(
        "schedflow.triggers.interval.get_localzone", Mock(return_value=timezone)
    )
    trigger = IntervalTrigger(
        start_date="2016-11-05 05:06:53", end_date="2017-11-05 05:11:32"
    )
    print("testt", type(timezone), trigger.start_date)
    r1 = localize(datetime(2016, 11, 5, 5, 6, 53), timezone)
    print("结果", repr(r1), repr(trigger.start_date))
    assert trigger.start_date == r1
...
def test_within(trigger, timezone):
    ...
    assert (
        trigger.get_next_fire_time(None, now) == trigger.start_date + trigger._interval
    )
