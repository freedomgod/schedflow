...
def test_get_next_fire_time(
    ...
    trigger = DateTrigger(run_date=run_date, timezone=alter_tz or timezone)
...
def test_repr(timezone):
    trigger = DateTrigger(run_date=datetime(2009, 7, 6), timezone=timezone)
...
def test_pickle(timezone):
    ...
    trigger = DateTrigger(run_date=date(2016, 4, 3), timezone=timezone)
