...
from schedflow.triggers.cron.expressions import (
    AllExpression,
...
def test_month_range_expression_single():
    expr = MonthRangeExpression("jan")
    assert str(expr) == "jan"
    assert repr(expr) == "MonthRangeExpression('jan')"


def test_month_range_expression_range():
    expr = MonthRangeExpression("mar", "jun")
    assert str(expr) == "mar-jun"


def test_month_range_expression_repr():
    expr = MonthRangeExpression("feb", "dec")
    assert repr(expr) == "MonthRangeExpression('feb', 'dec')"


def test_month_range_expression_invalid_name():
    with pytest.raises(ValueError, match='Invalid month name'):
        MonthRangeExpression("xyz")


def test_month_range_expression_invalid_last_name():
    with pytest.raises(ValueError, match='Invalid month name'):
        MonthRangeExpression("jan", "abc")
