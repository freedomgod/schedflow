...
下图是可交互的**高层架构总览**（单文件 HTML，支持明暗主题、缩放聚焦与导出）：

> [打开 SchedFlow 架构总览：定时工作流的创建、调度与执行](images/schedflow-architecture.html)

主路径一眼可见：**用户/调用方 → REST/SSE API → Scheduler → Executor → Workflow（DAG）→ ExecutionLog → JobStore**；认证、触发器与运行器作为侧翼组件挂接在主路径两侧，体现“单进程、多执行器/多存储器按 alias 路由”的轻量设计。
