...
> **文档**：[https://schedflow.readthedocs.io/](https://schedflow.readthedocs.io/)
...
生产模式默认不启用文件监听，避免 jobs.db 等运行期文件的写入被 uvicorn 热重载器当作“变更”打印，也不会因热重载产生多个调度器进程。开发模式下，`jobs.db`、`.git` 等文件已被加入重载排除列表，不会触发重启或刷屏。
